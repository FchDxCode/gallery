...
<ul class="flex justify-end">
    ...
    <div class="flex justify-center items-center">
        <div class="bg-[#2A3132] w-full max-w-md p-8 rounded-lg shadow-lg mt-8">
            <?php if ($user): ?>
                <div class="mb-6 text-center">
                    <label for="profile_photo" class="block text-sm font-medium text-white"></label>
                    <img src="./<?= $user['profile_photo'] ?>" alt="Foto Profil" class="rounded-full h-24 w-24 mx-auto shadow-lg">
                </div>
                <div class="mb-6">
                    <label for="username" class="block text-sm font-medium text-white">Username:</label>
                    <p id="username-display" class="text-lg font-semibold text-red-400"><?= $user['username'] ?></p>
                </div>
                <div class="mb-6">
                    <label for="fullname" class="block text-sm font-medium text-white">Full Name:</label>
                    <p id="fullname-display" class="text-lg font-semibold text-red-400"><?= $user['namalengkap'] ?></p>
                </div>
                <div class="mb-6">
                    <label for="email" class="block text-sm font-medium text-white">Email:</label>
                    <p id="email-display" class="text-lg font-semibold text-red-400"><?= $user['email'] ?></p>
                </div>
                <div class="mb-6">
                    <label for="address" class="block text-sm font-medium text-white">Address:</label>
                    <p id="address-display" class="text-lg font-semibold text-red-400"><?= $user['alamat'] ?></p>
                </div>
                <!-- Tampilkan jumlah foto yang sudah diunggah oleh pengguna -->
                <div class="mb-6">
                    <label for="total-photos" class="block text-sm font-medium text-white">Number of Photos Uploaded:</label>
                    <p id="total-photos-display" class="text-lg font-semibold text-red-400"><?= $total_photos ?></p>
                </div>
                <!-- Tombol Edit Profil -->
                <button id="edit-button" class="mt-4 w-full bg-red-500 text-white font-semibold py-2 px-4 rounded-md hover:bg-red-600">Edit Profil</button>
                    ...
                    <div class="popup bg-white">
                        ...
                        <form action="<?= $_SERVER['PHP_SELF'] ?>" method="POST" enctype="multipart/form-data">
                            <div class="mb-4">
                                ...
                                <input type="file" id="profile_photo" name="profile_photo" class="mt-1 p-2 block w-full border border-gray-300 rounded-md outline-none">
                            </div>
                            <div class="mb-4">
                                <label for="username" class="block text-sm font-medium text-gray-700">Username:</label>
                                <input type="text" id="username" name="username" value="<?= $user['username'] ?>" class="mt-1 p-2 block w-full border border-gray-300 rounded-md outline-none">
                            </div>
                            <div class="mb-4">
                                <label for="fullname" class="block text-sm font-medium text-gray-700">Full Name:</label>
                                <input type="text" id="fullname" name="fullname" value="<?= $user['namalengkap'] ?>" class="mt-1 p-2 block w-full border border-gray-300 rounded-md outline-none">
                            </div>
                            <div class="mb-4">
                                <label for="email" class="block text-sm font-medium text-gray-700">Email:</label>
                                <input type="email" id="email" name="email" value="<?= $user['email'] ?>" class="mt-1 p-2 block w-full border border-gray-300 rounded-md outline-none">
                            </div>
                            <div class="mb-4">
                                <label for="address" class="block text-sm font-medium text-gray-700">Address:</label>
                                <textarea id="address" name="address" class="mt-1 p-2 block w-full border border-gray-300 rounded-md outline-none"><?= $user['alamat'] ?></textarea>
                            </div>
                            <button type="submit" class="w-full bg-green-500 text-white py-2 px-4 rounded-md hover:bg-green-600">Save Changes</button>
                        </form>
                        <button id="close-popup" class="mt-4 w-full bg-red-500 text-white py-2 px-4 rounded-md hover:bg-red-600">Close</button>
                    </div>
                </div>
                <script>
